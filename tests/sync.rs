...
use filesync::diff::{diff, DiffOptions};
use filesync::manifest::{DstRoot, Kind, SrcRoot};
use filesync::plan::{plan, Action};
use filesync::progress_update::{CompareProgress, Progress};
...
fn sync_with(src: &Path, dst: &Path, opts: &Options) -> Report {
    ...
    let dopts = DiffOptions { relative_symlinks: opts.relative_symlinks, ..DiffOptions::default() };
    let df = diff(&s, &sm, &d, &dm, &dopts, &CompareProgress::hidden());
    let actions = plan(&df);
...
fn no_temp_files_remain_after_sync() {
    ...
    let (_, swept) = filesync::scan::scan_destination(&DstRoot::new(d.path()), &mut filesync::progress_update::ScanProgress::hidden());
    assert_eq!(swept, 0);
}
...
fn destination_scan_sweeps_leftover_temp_files_only() {
    ...
    let (outcome, swept) = filesync::scan::scan_destination(&DstRoot::new(d.path()), &mut filesync::progress_update::ScanProgress::hidden());
    assert_eq!(swept, 2);
    assert!(d.path().join("keep.txt").is_file());
